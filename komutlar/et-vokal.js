...
module.exports.run = async (client, message, args) => {
  if(!message.member.roles.cache.has(ayarlar.yetenekavcisi) && !message.member.hasPermission("ADMINISTRATOR") ) // ' 'nin arasına yetenek avcısı rol id
     return message.channel.send('Yetkin yok moruk').then(x => x.delete({timeout: 5000}));
   ...
   wasty.roles.add(ayarlar.vokal)
   let embed = new Discord.MessageEmbed()
   .setColor('CYAN')
   .setTitle('Vokal Rolü Verildi')
   .addField('**Vokal Yapılan Kullanıcı**',member)
   .addField('**Komutu Kullanan Yetkili**', message.author)
   .setFooter(ayarlar.owner)
